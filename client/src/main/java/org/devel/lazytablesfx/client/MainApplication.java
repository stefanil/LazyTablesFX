	...
	private Parent createNodes() {
		...
		Thread thread = new Thread(new Task<Void>() {
			...
			protected Void call() throws Exception {
				...
				WebTarget target = client.target("http://localhost:9000/").path("/people");
			...
			private void loadListData(List<Person> people) {
				listView.setBackground(new Background(new BackgroundFill(Color.AQUA, new CornerRadii(5), new Insets(5))));
